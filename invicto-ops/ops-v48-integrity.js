...
(function(){
  ...
  window.searchWarrantyClientV48=async function(){
    ...
    try{const {data,error}=await invictoSupabaseV12.rpc('search_ops_sales_v48',{p_query:q,p_limit:80});if(error)throw error;warrantySearchRows48=data||[];host.innerHTML=warrantySearchRows48.map(r=>{const own=admin()||String(r.advisor_id||'')===String(session?.id||'');return `<div class="v48-search-row"><div><b>${E(r.order||'Venta')}</b><small>${E(r.commercial_date||'—')} · ${E(r.source||'')} · ${E(r.sale_status||'')}</small></div><div><b>${E(r.customer_name||'Sin nombre')}</b><small>${E(r.customer_phone||'')}</small></div><div><span class="v48-guide">${E(r.tracking_number||'SIN GUÍA')}</span><small>${E(r.carrier||'')}</small></div><div><b>${E(r.advisor||'Sin asignar')}</b><small>${Number(r.attempts||0)} gestiones${r.confirmed_by_name?' · Confirmó: '+E(r.confirmed_by_name):''}</small></div><div>${own?`<button class="btn cyan sm" onclick="openWarrantyForSaleV48('${r.sale_id}')">Abrir garantía</button>`:'<span class="tag gray">OTRO ASESOR</span>'}</div></div>`}).join('')||'<div class="muted">No encontré ventas con ese dato.</div>';}
