/* INVICTO OPS v23.1 · historial mensual de ventas y dashboard individual */
const OPS_UI_VERSION_V23='23.1';
...
function isHistoricalV23(r){return r?.record_origin==='historical_shopify'}
function reportStatusLabelV23(st,reason=''){
  const m={nueva:'Nueva',asignada:'Nueva',en_gestion:'En gestión',no_contesta:'No contesta',seguimiento_programado:'Seguimiento futuro',pendiente_stock:'Pendiente stock',confirmada:'Confirmada',perdida:'Perdida',duplicada:'Duplicada',cancelada:'Cancelada',historica:'Histórica Shopify'};
...
function reportStatusTagV23(r){const st=reportStatusLabelV23(r.status,r.loss_reason),cls=r.status==='confirmada'?'green':r.status==='perdida'||r.status==='cancelada'?'red':r.status==='duplicada'||r.status==='historica'?'gray':r.status==='pendiente_stock'||r.status==='seguimiento_programado'?'amber':'blue';return `<span class="tag ${cls}">${esc(st)}</span>`}
...
function validConversionV23(r){return !isHistoricalV23(r)&&r.valid_for_conversion!==false&&r.status!=='duplicada'}
function pctV23(a,b){return b?Math.round((a-b)/Math.abs(b)*100):a?100:0}
function deltaHtmlV23(a,b){const d=pctV23(a,b),cls=d>0?'up':d<0?'down':'flat',sign=d>0?'+':'';return `<span class="month-delta ${cls}">${sign}${d}%</span>`}
function reportMetricsV23(rows){
  const historical=rows.filter(isHistoricalV23),live=rows.filter(r=>!isHistoricalV23(r)),valid=live.filter(validConversionV23),confirmed=live.filter(r=>r.status==='confirmada'),lost=live.filter(r=>r.status==='perdida'),drafts=live.filter(isDraftV23),pending=live.filter(r=>r.mapping_status==='pending');
  ...
  return {sales:rows.length,live:live.length,historical:historical.length,valid:valid.length,confirmed:confirmed.length,lost:lost.length,drafts:drafts.length,pending:pending.length,totalValue,confirmedValue,units,rate:valid.length?Math.round(confirmed.length/valid.length*100):0,aov:rows.length?Math.round(totalValue/rows.length):0};
}
...
function miniBarsV23(rows){
  const [y,m]=monthlySelectedV23.split('-').map(Number),days=new Date(Date.UTC(y,m,0)).getUTCDate(),by={};for(let d=1;d<=days;d++)by[d]={all:0,confirmed:0};
  rows.forEach(r=>{const d=Number(String(r.commercial_date).slice(8,10));if(by[d]){by[d].all++;if(r.status==='confirmada')by[d].confirmed++}});const max=Math.max(1,...Object.values(by).map(x=>x.all));
  return `<div class="monthly-chart-v23">${Object.entries(by).map(([d,x])=>`<div class="month-bar-col" title="Día ${d}: ${x.all} ventas · ${x.confirmed} confirmadas"><div class="month-bar-stack"><i class="month-bar-all" style="height:${Math.max(2,Math.round(x.all/max*100))}%"></i><i class="month-bar-confirmed" style="height:${Math.round(x.confirmed/max*100)}%"></i></div><small>${d}</small></div>`).join('')}</div><div class="month-chart-legend"><span><i></i>Ventas</span><span><i class="confirmed"></i>Confirmadas en OPS</span></div>`;
}
function groupRowsV23(rows,key){return rows.reduce((a,r)=>{const k=r[key]||'Sin definir';a[k]=(a[k]||0)+1;return a},{})}
function statLinesV23(obj,total){return Object.entries(obj).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<div class="statline"><span>${esc(reportStatusLabelV23(k))}</span><b>${v} <small class="muted">${total?Math.round(v/total*100):0}%</small></b></div>`).join('')||'<div class="muted">Sin datos.</div>'}
function monthlyCardsV23(cur,prev){
  const historicalPrev=prev.historical>0,confirmDelta=historicalPrev?'<span class="muted">Sin comparable pre-POS</span>':deltaHtmlV23(cur.confirmed,prev.confirmed);
  return `<div class="cards month-cards-v23">
    ...
    <div class="card"><small>Confirmadas en OPS</small><b>${cur.confirmed.toLocaleString('es-CO')}</b><div class="sub">${confirmDelta} · tasa ${cur.rate}%</div></div>
    ...
    <div class="card"><small>Valor confirmado OPS</small><b>${money(cur.confirmedValue)}</b><div class="sub">Solo desde puesta en marcha del POS</div></div>
    <div class="card"><small>Unidades</small><b>${cur.units.toLocaleString('es-CO')}</b><div class="sub">ticket prom. ${money(cur.aov)}</div></div>
    <div class="card"><small>Histórico Shopify</small><b>${cur.historical}</b><div class="sub">${cur.drafts} borradores · ${cur.pending} por surtir</div></div>
  </div>`;
}
