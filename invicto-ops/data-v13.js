  ...
  try{
    ...
    state.sales=sales.map(s=>{
      ...
      return {
        ...
        confirmedAt:s.confirmed_at,confirmedBy:s.confirmed_by,confirmedByName:pm.get(s.confirmed_by)?.full_name||'',stockReservationItems:r.map(x=>({warehouse:wm.get(x.warehouse_id)?.name||'',warehouseId:x.warehouse_id,productVariantId:x.product_variant_id,variantKey:canonicalVariantKey(vm.get(x.product_variant_id)?.category||'',vm.get(x.product_variant_id)?.material||'',vm.get(x.product_variant_id)?.style||'',vm.get(x.product_variant_id)?.size||'',vm.get(x.product_variant_id)?.design_or_color||''),qty:Number(x.quantity||0)})),
