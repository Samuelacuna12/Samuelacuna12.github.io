...
(function(){
  ...
  function stats(){
    ...
    return {ms,target,counted,remaining,required,projection,pct,elapsed,totalDays,shopify:n(d?.shopify_orders),document:n(d?.document_sales)};
  }
  ...
  window.goalPanelV18=function(){
    ...
    const g=stats();
    return `<div class="panel" style="margin-bottom:14px"><div class="panel-head"><div><h3>Meta mensual · ${f(g.target)} ventas</h3><div class="muted"><b>${f(g.counted)}</b> ventas contabilizadas · <b>${g.pct.toFixed(1)}%</b> de cumplimiento</div><div class="muted" style="margin-top:4px">Documento Invicto es el conteo oficial. Shopify se muestra aparte y no se suma nuevamente.</div></div><div class="spacer"></div>${isAdmin()?'<button class="btn light sm" onclick="setMonthlyTargetV18()">Editar meta</button>':''}</div><div class="panel-body"><div style="height:12px;background:#e8edf3;border-radius:99px;overflow:hidden"><div style="height:100%;width:${Math.min(100,g.pct)}%;background:#00cfa8"></div></div><div class="mini-grid" style="margin-top:14px"><div class="mini"><small>Documento Invicto</small><b>${f(g.document)}</b><div class="muted">conteo oficial para la meta</div></div><div class="mini"><small>Shopify</small><b>${f(g.shopify)}</b><div class="muted">pedidos del mes · separado</div></div><div class="mini"><small>Faltan</small><b>${f(g.remaining)}</b><div class="muted">para llegar a la meta</div></div><div class="mini"><small>Necesarias / día</small><b>${f(g.required)}</b><div class="muted">días hábiles Lun–Sáb restantes</div></div><div class="mini"><small>Proyección</small><b>${f(g.projection)}</b><div class="muted">al cierre del mes</div></div><div class="mini"><small>Días operativos</small><b>${g.elapsed}/${g.totalDays}</b><div class="muted">transcurridos</div></div></div></div></div>`;
  };
  async function load(force){
    if(loading||typeof invictoSupabaseV12==='undefined')return;
    if(!force&&d&&Date.now()-last<120000)return;
    loading=true;
