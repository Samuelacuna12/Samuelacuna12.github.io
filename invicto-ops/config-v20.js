/* INVICTO OPS v23.1 · estado de integraciones y configuración productiva */
async function integrationStatusV20(){
  const out={logiCities:0,exports:0,target:0,history:0,webhookErrors:0};
  try{
    const [a,b,c,d,e]=await Promise.all([
      ...
      invictoSupabaseV12.from('shopify_sales_history').select('id',{count:'exact',head:true}),
      invictoSupabaseV12.from('shopify_webhook_events').select('webhook_id',{count:'exact',head:true}).eq('status','error')
    ]);
    out.logiCities=a.count||0;out.exports=b.count||0;out.target=c.data?.[0]?.target_confirmed_sales||0;out.history=d.count||0;out.webhookErrors=e.count||0;
...
window.renderConfig=async function(){
  ...
  const st=await integrationStatusV20(),aiStatus=aiConfiguredV19===true?'ACTIVA':aiConfiguredV19===false?'FALTA API KEY':'POR VERIFICAR',aiCls=aiConfiguredV19===true?'green':aiConfiguredV19===false?'red':'amber';
  host.innerHTML=`<div class="page-title"><div><h1>Configuración e integraciones</h1><p>Estado productivo real de INVICTO OPS v23.1.</p></div><span class="tag green">PRODUCCIÓN</span></div>
  <div class="cards" style="grid-template-columns:repeat(3,minmax(170px,1fr))">
    ${integrationCardV20('Supabase','CONECTADO','PostgreSQL, Auth, RLS, inventario y funciones operativas','green')}
    ${integrationCardV20('Shopify Webhooks',st.webhookErrors?'REVISAR':'EN VIVO',st.webhookErrors?`${st.webhookErrors} evento(s) en error`:'Pedidos + borradores + cancelaciones · webhook v8',st.webhookErrors?'red':'green')}
    ${integrationCardV20('Histórico Shopify',st.history?'CARGADO':'CREDENCIAL PENDIENTE',st.history?`${st.history.toLocaleString('es-CO')} pedidos históricos`:'Sin Admin API token en Supabase para backfill automático',st.history?'green':'amber')}
    ...
    ${integrationCardV20('MASSIVE LogiGho',st.logiCities?'LISTO':'CATÁLOGO PENDIENTE',st.logiCities?`${st.logiCities} códigos de destino cargados`:'Falta catálogo oficial ciudad/departamento → código LogiGho',st.logiCities?'green':'amber')}
    ${integrationCardV20('Hoko API','PENDIENTE','Faltan credenciales y especificación técnica utilizable de endpoints','amber')}
  </div>
  ...
  <div class="panel" style="margin-top:14px"><div class="panel-head"><h3>Arquitectura activa</h3></div><div class="panel-body"><p class="muted" style="line-height:1.6;margin:0">Shopify → Webhooks firmados → Supabase Edge Functions → PostgreSQL/RLS → INVICTO OPS. Inventario, reservas, recuperación, cortes, logística, garantías, novedades y KPIs usan la misma base. El histórico anterior al POS vive en una tabla separada y nunca genera SLA, asignaciones ni movimientos de stock.</p></div></div>`;
};
